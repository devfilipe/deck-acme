...
arrives from `packs/_workspaces/all/default/config/*.yaml` as declarative data.
...
```
packs/_workspaces/all/default/   ← the base layer: every repository, every scope
├── config/
...
The gate commands in `packs/_workspaces/all/default/config/gates.yaml` echo, or call a
...
Copy `packs/_workspaces/all/default/`, keep the structure, replace the content:
