...
cat <<BANNER
  ...
  The carve-up below is not this machine's opinion: it arrives in
    packs/_workspace/templates/workspace/workspace.yaml
...
run pack new checkout-ops --scope checkout --from-workspace \
  --dir "$WS/packs/checkout-ops" \
...
run ask fold "$ASK" --as toggle --into "$WS/packs/checkout-ops" || true
...
run ask fold "$ASK" --as toggle --into "$WS/packs/checkout-ops" \
...
printf '\n\033[2m$ sed -n "/^# Folded from/,\$p" packs/checkout-ops/config/toggles.yaml\033[0m\n'
sed -n '/^# Folded from/,$p' "$WS/packs/checkout-ops/config/toggles.yaml"

say "10. It is a decision now, not a transcript"
run --scope checkout toggle explain retry_without_key
run pack validate "$WS/packs/checkout-ops"
